(function () {
  ...
  function run() {
    commander
      ...
      .option("--jsdox", "exec jsdox command", DEFAULTS.jsdox)
      .parse(process.argv)
      .name = 'makejs';
    ...
    var timeoutId
      , run = function (sources, options) {
        return Deferred
          ...
          .next(function () {
            if (options.jsdox) {
              generateJSDox(sources, options.jsdox);
            }
          })
          .next(function () {
            if (options.docco) {
              generateDocco(sources, options.docco);
            }
          })
          .error(function (err) {
            util.stack(err);
  ...
  function generateJSDox(sources, options) {
    dox
      .document(sources, options)
      .error(function (err) {
        util.stack(err);
      });
  }
  ...
  function generateDocco() {
    var dfd = new Deferred()
      , filenames = findFiles(libraryDir)
      ;

    docco.document(filenames, {
      output: 'docs'
    }, function () {
      dfd.call();
    });
